...
void		draw(t_mlx *m, int *a, int x)
{
	...
	while (y < WIN_HEIGHT)
	{
		...
		while (y >= a[1] && y < a[2])
		{
			...
			color = m->tex.col[a[3]][64 * texy + m->p.texx];
...
void		find_lineheight(t_mlx *m, int side, int x)
{
	...
	a[0] = (int)(WIN_HEIGHT / m->p.pwalldist);
	a[1] = -a[0] / 2 + WIN_HEIGHT / 2;
	(a[1] < 0) ? a[1] = 0 : 0;
	a[2] = a[0] / 2 + WIN_HEIGHT / 2;
	(a[2] >= WIN_HEIGHT) ? a[2] = WIN_HEIGHT - 1 : 0;
	a[3] = 1;
	(side == 0 && m->p.raydirx < 0) ? a[3] = 2 : 0;
	(side == 1 && m->p.raydiry < 0) ? a[3] = 3 : 0;
	(side == 1 && m->p.raydiry > 0) ? a[3] = 0 : 0;
	draw(m, a, x);
}
